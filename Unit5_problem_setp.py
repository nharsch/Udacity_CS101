...
def when_offered(courses, course):
	return [h for h in courses if course in courses[h]]
	#this brilliant trick I learned from the discussions
...
def hashtable_update(htable, key, value):
    ...
    if bucket_find(bucket, key):
    	bucket_find(bucket, key)[1] = value
...
def cached_execution(cache, proc, proc_input):
	# format of cache { proc_input1:proc_output1, etc.}
	pname = proc.__name__
